...
状态栏还会显示本地对象库用量和配置的空间预算。默认空间目标为 10GB，默认历史
保留期为 7 天；自动清理默认关闭，必须在“存储”页面明确启用。
...
“历史版本自动清理”会显示预计删除的版本、快照、对象数量和可释放空间。首次启用或
缩短保留期必须输入 `ENABLE AUTO CLEANUP`。启用后后台每天最多执行一次，只删除超过
保留期且已被新版本取代的历史；每个文件最后一个可恢复版本、重要恢复点和 AI 修改
恢复点始终保留。
